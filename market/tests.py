...
class ProductListViewTests(TestCase):
    ...
    def test_product_list_shows_active_products(self):
        response = self.client.get(reverse('market:productlist'))
    ...
    def test_product_list_search_by_title(self):
        response = self.client.get(reverse('market:productlist') + '?q=iPhone')
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Apple iPhone')
        self.assertNotContains(response, 'Samsung Galaxy')

    def test_product_list_filter_by_category(self):
        response = self.client.get(reverse('market:productlist') + '?category=electronics')
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Apple iPhone')
        self.assertContains(response, 'Samsung Galaxy')

    def test_product_list_order_by_price_asc(self):
        response = self.client.get(reverse('market:productlist') + '?order=asc')
        self.assertEqual(response.status_code, 200)
...
class ProductDetailViewTests(TestCase):
    ...
    def test_product_detail_view(self):
        response = self.client.get(reverse('market:product-detail', args=[self.product.id]))
        self.assertEqual(response.status_code, 200)
...
class AddToCartViewTests(TestCase):
    ...
    def test_add_to_cart_creates_cart_and_item(self):
        response = self.client.post(reverse('market:add-to-cart', args=[self.product.id]))
...
class CartViewTests(TestCase):
    ...
    def test_view_cart_creates_empty_cart(self):
        response = self.client.get(reverse('market:view-cart'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_cart_increase_quantity(self):
        cart = Cart.objects.create(user=self.user)
        CartItem.objects.create(cart=cart, product=self.product, quantity=2)
        response = self.client.post(reverse('market:cart-increase', args=[self.product.id]))
        self.assertEqual(response.status_code, 302)
        item = cart.items.first()
        self.assertEqual(item.quantity, 3)
    ...
    def test_cart_decrease_quantity(self):
        cart = Cart.objects.create(user=self.user)
        CartItem.objects.create(cart=cart, product=self.product, quantity=3)
        response = self.client.post(reverse('market:cart-decrease', args=[self.product.id]))
        self.assertEqual(response.status_code, 302)
        item = cart.items.first()
        self.assertEqual(item.quantity, 2)
